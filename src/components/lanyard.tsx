...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }: BandProps) {
  ...
  useFrame((state, delta) => {
    if (!mounted || !card.current || !j1.current || !j2.current || !j3.current || !fixed.current || !band.current) {
      return;
    }
    ...
    try {
      ...
      console.error('Error in lanyard animation frame:', error);
    }
  });
  ...
  if (!nodes?.card || !materials?.base || !materials?.metal) {
    return null;
  }
  ...
  return (
    <>
      ...
      <mesh ref={band}>
        {/* @ts-expect-error - meshline types are registered via extend() */}
        <meshLineGeometry />
        {/* @ts-expect-error - meshline types are registered via extend() */}
        <meshLineMaterial
          color="white"
